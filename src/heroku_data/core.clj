...
(defn get-heroku-data [url options]
  (let [{:keys [body error] :as resp} @(http/get url options)]
    (if error
...
(def org-url (str api-url "/organizations/"))
...
(def env-app-vars (json2map (map #(get-heroku-data (str api-url "/apps/" % "/config-vars") options)
                                 (map :name app-names))))
